...
class tmpFile(object):
    ...
    def __init__(self, ext='', path='./', destory=True):

        self.__tmpFileName = abspath(join(path, uuid4().hex.upper() + ext))
        self.__willDestory = bool(destory)
    ...
    def __exit__(self, *exc_info):
        ...
        if exists(self.__tmpFileName) and self.__willDestory:
